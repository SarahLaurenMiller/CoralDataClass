...
var queryUrl = "https://raw.githubusercontent.com/nancyecampos/sandbox/master/2018.geojson";

// Perform a GET request to the query URL
d3.json(queryUrl, function (data) {
  createMap(data.features);
});
function assignColor(coralType){
  if (coralType === "demosponge"){
    return 'orange'
  }else if(coralType === "glass sponge"){
    return 'blue'
  }else{
    return 'grey'
  }
...
function createMap(corals) {
  ...
  for (coral of corals) {
    ...
    L.circleMarker(latlon, { // create the circle
      ...
      color: assignColor(coral.properties.commonname), 
      radius: 1.0
